...
索引队列采用本地成熟方案：SQLite durable queue 负责持久化、恢复、取消、重试和进度；main process 的 `IndexingQueueService` 只做调度；`worker_threads` 执行解析/chunk 这类 CPU 工作。现在的 worker 先支持 md/txt/code 的文本解析和 chunk 统计，PDF/DOCX/PPTX/image 会生成明确 warning，后续接 parser adapter、embedding limiter 和 LanceDB vector upsert。
...
- SQLite indexing queue 已接入：`importFiles` / `indexCourseFiles` 创建 durable job/task，app 启动会恢复 expired running task，main scheduler 通过 `worker_threads` 跑基础 parse/chunk worker；embedding limiter 和 LanceDB upsert 是下一步。
