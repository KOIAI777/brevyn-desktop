...
- Claude Agent SDK（`@anthropic-ai/claude-agent-sdk`）用于 Agent loop、streaming、tools、permissions 和 human review。SDK 在 main 进程内 spawn 一个 claude CLI 子进程，UCLAW 通过 `query()` 接口与之交互，并由 UCLAW 自己把 session 事件追加写入 JSONL。
...
- Proma 也使用 Claude Agent SDK，验证了 Anthropic 协议路线在桌面 Agent 产品里的可行性，特别是国产 provider（DeepSeek、Kimi、智谱）通过 Anthropic 兼容层接入的模式。UCLAW 直接复用 Proma 的 SDK 调用模板和 baseUrl 规范化逻辑。

UCLAW 不应照搬 Proma 的通用 Agent 产品形态，而应复用它的运行时分层和 Claude SDK 接入方式。
...
- Threads：Home session、task session。Course container 只承载课程文件和任务，不创建 course-level session。
  ...
  src/
    main/
      ...
      agent/                          # 新增：agent runtime 全部聚合在这
        ...
        tool-registry.ts              # UCLAW 自建 function tools（MVP 只注册 rag_search）
      services/
    ...
    renderer/
      ...
      components/
        ...
        courses/                       # CourseManagementDialog
...
不在本期范围（之前过度设计的 service，全部从目录里删掉）：`git-service.ts` / `shell-service.ts` / `dialog-service.ts` / `channel-service.ts` / `mcp-service.ts` / `edit-service.ts` —— 这些能力 Claude Agent SDK 自带 Read/Write/Edit/Bash/Glob/Grep 等内置工具，UCLAW 不再自建。
...
SQLite 业务表从这些开始。`src/main/storage/sqlite-business-store.ts` 是全新 baseline；不再读取 `uclaw-state.json`，不做 legacy JSON 导入，也不在读路径里自动 seed / repair 业务数据。Provider 不在 SQLite，`providers` 表启动时会被删除：
...
- Agent 工具能力由 Claude Agent SDK 内置 + UCLAW MCP server 提供，无需 provider 级开关。
...
参考 Proma 的做法，Skill 不作为模型可调用的 `list_skills` 工具暴露。Proma 扫描工作区 `skills/` / `skills-inactive/` 下的 `SKILL.md` 做启用状态，并把启用 skill 的内容拼入 Claude Agent SDK 的 `systemPrompt` 选项。UCLAW 沿用这一思路：`SkillService` 只负责文件型 skill 的注册、启停、读取和 prompt 注入；启用 skill 的摘要由 `prompt-builder.ts` 在 run 启动时一次性塞进 system prompt，模型不需要单独的 skill listing tool。
...
UCLAW 通过 SDK 的 `query()` 选项控制行为：

- `cwd`：限定本次 run 的工作目录到当前 thread 的课程/任务目录，让 Read/Write/Bash 默认只能在此根下活动。
- `allowedTools` / `disallowedTools`：白/黑名单。MVP 默认 allow `Read` `Glob` `Grep` `TodoWrite` 以及 UCLAW 自建的 `rag_search`；review 模式下 `Write` `Edit` `Bash` `WebFetch` 走 `canUseTool` 弹 approval；`WebSearch` 默认 disallow（国产 provider 不支持 hosted web search）。
- `canUseTool`：每次 tool 调用前的拦截回调，UCLAW `PermissionService` 实现这个回调，返回 `{ behavior: "allow" | "deny", message? }`。
...
- 所有 `Bash` 命令通过 SDK 默认就是非交互、可中断、可截断输出，UCLAW 只在事件流里把 tool_use/tool_result 记录到 thread JSONL。
...
MVP 不自建 token 估算 service。Claude Agent SDK 自己管理 turn budget、自动 compaction（`/compact`）和上下文限流；`SDKResultMessage` 会带 `usage`（input/output/cache tokens），UCLAW 把这些信息直接写入 JSONL 的 `response_metrics` 事件并展示在 UI 里。
...
UCLAW 的事件命名保持稳定（renderer 不感知底层换 SDK），由 main 进程的 `claude-sdk-adapter` 把 Claude Agent SDK 的 `SDKMessage` 流映射成 UCLAW 自己的 item：
...
UCLAW 的 chat / agent UI 已经被移除，本期需要重新接入。直接参考 Proma 0.9.11 的实现而不是再造：
...
UCLAW 的 timeline 归一化层（`renderer/lib/timeline.ts`）和 `taskagent-sweep-text` / `taskagent-panel-content-in` 动画也需要重新写一份简化版（之前的版本已经删除）。
...
window.uclaw.semester.create(input)
window.uclaw.semester.select(semesterId)
window.uclaw.semester.archive(semesterId)
window.uclaw.semester.restore(semesterId)
window.uclaw.semester.delete(semesterId)

window.uclaw.courses.list()
window.uclaw.courses.listArchived({ semesterId? })
window.uclaw.courses.create(input)
window.uclaw.courses.archive(courseId)
window.uclaw.courses.restore(courseId)
window.uclaw.courses.delete(courseId)

window.uclaw.tasks.list(courseId)
window.uclaw.tasks.create(input)
window.uclaw.tasks.delete(taskId)

window.uclaw.threads.list(courseId?)
window.uclaw.threads.listArchived({ semesterId?, courseId? })
window.uclaw.threads.create(input)
window.uclaw.threads.archive(threadId)
window.uclaw.threads.restore(threadId)
window.uclaw.threads.delete(threadId)

window.uclaw.files.tree(courseId?)
window.uclaw.files.preview(fileId)
window.uclaw.files.import(input)
window.uclaw.files.sections(courseId)
window.uclaw.files.stats(courseId?)
window.uclaw.files.index(courseId, sectionId?)
window.uclaw.files.indexingJobs(courseId?)
window.uclaw.files.cancelIndexing(jobId)
window.uclaw.files.delete(fileId)
window.uclaw.files.reveal(fileId)

window.uclaw.providers.list()
window.uclaw.providers.save(input)
window.uclaw.providers.delete(providerId)
window.uclaw.providers.models(providerId)
window.uclaw.providers.test(providerId)

window.uclaw.skills.list()
window.uclaw.skills.update(input)
window.uclaw.skills.readContent(skillId)
window.uclaw.skills.writeContent(input)
window.uclaw.skills.importFolder(input)
window.uclaw.skills.openFolder(skillId)

window.uclaw.rag.search(query, courseId?)
window.uclaw.git.status()
window.uclaw.timetable.range(query)
window.uclaw.app.openExternal(url)
```

Agent API 还没接回。后续补 runtime 时再新增 `window.uclaw.agent.run/stop/approve/reject/onEvent`；Renderer 只订阅 `agent.onEvent`，不自己拼 tool state。

### Agent Tool Surface

Agent 可调用能力单独注册，不从 `window.uclaw.*` 暴露。详细工具清单见下文 `## Claude Agent SDK 集成` 章节。简言之分三类：

- Claude Agent SDK 内置工具：`Read` / `Write` / `Edit` / `Glob` / `Grep` / `Bash` / `WebFetch` / `TodoWrite`。
- UCLAW 自建 function tool（通过 in-process MCP server 注入）：MVP 只有 `rag_search` 一个。
...
UCLAW 使用 `@anthropic-ai/claude-agent-sdk`。SDK 把 Anthropic 协议封装在一个本地 claude CLI 子进程里，UCLAW main process 通过 `query()` 接口流式拿到消息。
...
const stream = sdk.query({
  ...
  options: {
    ...
    mcpServers: uclawMcp.toMcpServerMap(),  // 本地 in-process MCP server，MVP 只注入 rag_search
    spawnClaudeCodeProcess: trackedSpawn,   // 自定义 spawn，跟踪 PID 用于强杀
    settingSources: ["user", "project"],    // 不加载用户全局 ~/.claude/settings.json 的全部
    toolUseConcurrency: 1,                  // 国产兼容层稳健性，串行 tool_use
  },
});

for await (const msg of stream) {
  // 映射到 UclawRunStreamItem，写 JSONL，emit 给 renderer
}
```
...
UCLAW **不需要**像 OpenAI 路线那样自建 `shellTool()` / `applyPatchTool()`，SDK 已经全部包含。

### UCLAW 自建 function tools
...
UCLAW 选择把模型层绑到 Claude Agent SDK 一条路径上，用 SDK 自带的本地工具集，避免维护多套 tool loop / approval / session。Claude Agent SDK 的本地工具体系（Read/Write/Edit/Bash/Glob/Grep）正好是 UCLAW 需要的；其他模型生态不进入当前 agent 编排主路径。
...
Claude Agent SDK 通过 `canUseTool` 回调表达权限询问。UCLAW `PermissionService` 实现这个回调：
...
- `types/domain.ts` 加回 `RunStatus` / `ChatMessage` / `UclawRunStreamItem` / `ToolCallPayload` / `ApprovalRequest` / `AgentRunInput` / `AgentRuntimeStatus` / `PermissionMode`。
- `types/ipc.ts` 加回 agent 相关 channel。
- `preload/index.ts` 加回 `window.uclaw.agent.*` bridge。
...
- 实现 `agent/agent-orchestrator.ts`：run lifecycle、并发守卫、错误处理、`SDKMessage` → `UclawRunStreamItem` 映射。
