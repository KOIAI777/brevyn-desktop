...
- Claude Agent SDK（`@anthropic-ai/claude-agent-sdk`）用于 Agent loop、streaming、tools、permissions、human review 和 session 持久化。SDK 在 main 进程内 spawn 一个 claude CLI 子进程，UCLAW 通过 `query()` 接口与之交互。
...
apps/uclaw-electron/
  ...
  src/
    main/
      ...
      services/
        course-service.ts             # 从 local-store 拆出：semester/course/task/thread
        file-service.ts               # 从 local-store 拆出：文件树、导入、预览、sections
        workspace-paths.ts            # 纯函数：ensureSemesterDir / ensureCourseDir 等
        provider-config-store.ts      # provider 配置（chat / embedding 共用一张表）
...
Provider 配置不再放 SQLite 业务库，单独由 `provider-profiles.json` + `provider-secrets.json` 管：
...
window.uclaw.skills.toggle(skillId, enabled)
window.uclaw.skills.readContent(skillId)
window.uclaw.skills.writeContent(skillId, content)
window.uclaw.skills.importFolder(sourcePath)
...
- ✅ 本地 Semester / Course / Task / Thread / WorkspaceFile / TimetableEvent / Skill 数据模型 + SQLite 业务表。
- ✅ Provider config + secret store（`purpose/kind/selectedModel/models[]` 双轨已经在数据层就位）。
...
### Stage 1 — 拆 LocalStore（还旧债）

`local-store.ts` 现在 ~1990 行，是个 God Object，会拖累后续 agent 接入。先按职责拆：

- 提取 `course-service.ts`：semester / course / task / thread CRUD。
- 提取 `file-service.ts`：文件树 / 导入 / 预览 / sections。
- 提取 `workspace-paths.ts`：纯函数 `ensureSemesterDir` / `ensureCourseDir` / `ensureTaskDir` / `sanitizeFsSegment` 等。
- `local-store.ts` 只保留 `load()` 入口和 `LocalStore` 协调对象（预计 300–500 行）。

验证：`npm run typecheck` + `npm run build` + `npm run dev` 全过；UI 各业务功能不变。
...
- `main/ipc.ts` 注册 stub handler（暂时返回空数据 / no-op）。
