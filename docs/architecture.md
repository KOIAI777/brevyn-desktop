...
Skills 是课程/工作区级能力包，用来给 Agent 注入稳定的方法、提示词和操作准则。Skill 不是 Tool；Tool 是模型可调用能力，Skill 是本地 `SKILL.md` 文档包。

参考 Proma 和 OpenAI Agents SDK，Skill 不作为模型可调用的 `list_skills` 工具暴露。Proma 是扫描工作区 `skills/` / `skills-inactive/` 下的 `SKILL.md` 做启用状态；OpenAI Agents SDK 则把能力放进 Agent 的 `instructions`、`tools`、`mcpServers` 和 run context。UCLAW 的 SkillService 因此只负责文件型 skill 的注册、启停、读取和 prompt 注入；模型需要了解当前能力时通过 `context_report` 看到 enabled skills 摘要，而不是调用单独的 skill listing tool。
...
window.uclaw.skills.readContent(courseId, skillId)
window.uclaw.skills.writeContent(courseId, skillId, content)
window.uclaw.skills.importFolder(courseId, sourcePath)
window.uclaw.skills.openFolder(courseId, skillId)
...
Agent 可调用能力单独注册，不从 `window.uclaw.*` 暴露：

- OpenAI hosted tools：`webSearchTool`、`fileSearchTool`、`codeInterpreterTool`、`imageGenerationTool`、`toolSearchTool`、`hostedMcpTool`。
- OpenAI SDK / local execution tools：`shellTool`、`applyPatchTool`，后续可评估 experimental `codexTool` 或 sandbox capabilities。
- UCLAW function tools：`context_report`、`rag_search`、`ask_user`、`web_fetch`。
- MCP tools：本地或远程 MCP server 暴露的工具。

Skill 创建/编辑不是单独的默认 Agent tool。自然语言里“帮我写一个 skill”时，Agent 应通过 shell/apply_patch 在对应 `skills/<slug>/SKILL.md` 和 `scripts/` 下创建文件；设置页的 `readContent/writeContent/importFolder` 只是人类 UI 的管理 API。
...
实现状态：Provider 设置页已经保存 `agentTools` 到 SQLite `providers.config_json`，OpenAI Responses provider 会把这些设置传给 `UclawToolRegistry`。OpenAI-compatible / Chat Completions 路径默认不启用 hosted tools，避免把 Responses-only 工具传给不支持的 provider。
...
- OpenAI SDK `Session` 已接到 thread JSONL：`sdk_session_item` / `sdk_session_pop` / `sdk_session_clear` 作为 SDK history 真源，UI 消息和 run events 仍保留现有 JSONL entry。
- Proma-style file skill skeleton 已接入：启动时写入 `default-skills/*/SKILL.md` 模板，并把默认 skill seed 到全局 `skills/skills-inactive`；运行时只扫描全局与课程级 `SKILL.md` 目录，SQLite `skills` 旧表不再参与加载或保存。
