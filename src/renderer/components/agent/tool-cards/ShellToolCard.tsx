...
export function ShellToolCard({
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-border/70 bg-card/82 px-4 py-3 font-mono text-[11px] leading-5 text-foreground shadow-sm ring-1 ring-white/45 backdrop-blur-xl">
      ...
      <div className={`${output ? "mt-3 grid-rows-[1fr] opacity-100" : "mt-0 grid-rows-[0fr] opacity-0"} grid transition-[grid-template-rows,opacity,margin] duration-220 ease-out`}>
        <div className="min-h-0 overflow-hidden">
          <pre className="max-h-60 overflow-auto whitespace-pre-wrap break-words text-muted-foreground brevyn-scrollbar">
            {output ? helpers.truncatePreview(output) : ""}
          </pre>
        </div>
      </div>
      <div className="mt-4 flex justify-end font-sans text-xs text-muted-foreground">
