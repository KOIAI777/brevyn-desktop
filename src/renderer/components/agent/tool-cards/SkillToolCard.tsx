...
import { CompactProcessCard, PreviewPill, ProcessCardHeader } from "./shared";
...
export function SkillToolCard({
  ...
  return (
    <div className="overflow-hidden border-l border-border/60 py-1 pl-3 text-xs text-foreground">
      <ProcessCardHeader
        title={<span className="inline-flex min-w-0 items-center gap-2">{helpers.renderToolGlyph("mcp__brevyn__load_skill", "h-3.5 w-3.5 shrink-0")}<span className="min-w-0 truncate">{title}</span></span>}
        collapsed={collapsed}
        onToggleCollapsed={onToggleCollapsed}
      />
