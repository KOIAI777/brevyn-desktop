...
export function formatEnabledSkillPrompt(skills: SkillItem[]): string {
  ...
  return enabled
    .map((skill) => {
      const scopeLabel = skill.scope === "course" ? "course" : "default";
      const instructions = skill.instructions?.trim() || "Use this skill when the task matches its scope.";
      return [`- ${skill.name} [${scopeLabel}]`, `  ${instructions}`].join("\n");
    })
    .join("\n");
}
