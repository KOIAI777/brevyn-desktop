...
export class PromptBuilder {
  buildSystemPrompt(context: AgentPromptContext): string {
    ...
    const lines = [
      ...
      "",
      "## Tool Strategy",
      "- Use Brevyn MCP tools for structured course workspace metadata. Exact tool names are mcp__brevyn__course_structure for semantic folders, mcp__brevyn__list_course_files for file records, mcp__brevyn__get_file_record for a known file id, and mcp__brevyn__propose_external_source for external web sources that should wait for user confirmation.",
      ...
      "- If WebSearch/WebFetch finds a useful external URL for the current course or task, do not silently treat it as course material. Immediately call propose_external_source with the URL, title, and a concrete reason; that tool is the user-confirmation inbox, so do not ask in chat whether to save it first.",
      "- After proposing external sources, briefly tell the user the sources are waiting in Brevyn's confirmation card. Do not list candidates as a substitute for calling propose_external_source.",
