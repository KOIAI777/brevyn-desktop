...
export type ReferenceItemType = "article-journal" | "book" | "chapter" | "paper-conference" | "report" | "webpage" | "video" | "thesis" | "document";
export type ReferenceSourceKind = "manual" | "import" | "doi_lookup" | "agent_search" | "course_material";
export type ReferenceCreatorRole = "author" | "editor" | "translator";
export type ReferenceScopeType = "semester" | "course" | "task" | "candidate";
export type ReferenceScopeStatus = "active" | "candidate" | "rejected";
export type ReferenceImportFormat = "bibtex" | "ris" | "csl-json";
export type ReferenceExportFormat = "bibtex" | "ris" | "csl-json" | "apa-markdown";
...
export interface ReferenceScopeQuery {
  semesterId?: string;
  courseId?: string;
  taskId?: string;
...
export interface ReferenceCreateInput {
  ...
  tags?: string[];
  rawCslJson?: Record<string, unknown>;
  scope?: Omit<ReferenceScope, "id" | "referenceId" | "createdAt">;
}

export interface ReferenceUpdateInput extends Partial<Omit<ReferenceCreateInput, "scope">> {
  id: string;
}

export interface ReferenceScopeInput {
  referenceId: string;
  scopeType: ReferenceScopeType;
  semesterId?: string;
  courseId?: string;
...
export interface ReferenceImportInput {
  ...
  content: string;
  scope?: Omit<ReferenceScopeInput, "referenceId">;
}
...
export interface BrevynAPI {
  ...
  references: {
    list: (query?: ReferenceScopeQuery) => Promise<ReferenceItem[]>;
    create: (input: ReferenceCreateInput) => Promise<ReferenceItem>;
    update: (input: ReferenceUpdateInput) => Promise<ReferenceItem>;
    archive: (referenceId: string) => Promise<boolean>;
    delete: (referenceId: string) => Promise<boolean>;
    addScope: (input: ReferenceScopeInput) => Promise<ReferenceScope>;
    removeScope: (scopeId: string) => Promise<boolean>;
    import: (input: ReferenceImportInput) => Promise<ReferenceImportResult>;
    export: (input: ReferenceExportInput) => Promise<ReferenceExportResult>;
  };
  git: {
